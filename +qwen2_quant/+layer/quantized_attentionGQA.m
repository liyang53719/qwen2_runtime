function [X_out, past_key_value, X_out_packed, trace] = quantized_attentionGQA(X, past_key_value, weights, freqs_cis, hyperParameters, cfg)
% quantized_attentionGQA   Grouped Query Attention with quantized linear projections
    ...
    if strcmpi(string(cfg.LinearMode), "mixed_dynamic")
        projMeta = struct('HeadDim', headDim, 'GroupSize', headDim);
        if isPackedActivation
            X_proj = dequantizePacked2D(X2_packed);
        else
            X_proj = X2;
        end
        projMeta.OpName = sprintf('h%d.attn.q_proj', layerIdx);
        projMeta.LayerIndex = layerIdx;
        xq = qwen2_quant.layer.linear_dispatch(weights.q_proj, X_proj, cfg, projMeta);
        projMeta.OpName = sprintf('h%d.attn.k_proj', layerIdx);
        xk = qwen2_quant.layer.linear_dispatch(weights.k_proj, X_proj, cfg, projMeta);
        projMeta.OpName = sprintf('h%d.attn.v_proj', layerIdx);
        xv = qwen2_quant.layer.linear_dispatch(weights.v_proj, X_proj, cfg, projMeta);
        xq_packed = [];
        xk_packed = [];
        xv_packed = [];
    elseif isPackedActivation
    ...
    trace.attn_mix = attn_output_cat;

    if strcmpi(string(cfg.LinearMode), "mixed_dynamic")
        projMeta = struct('HeadDim', headDim, 'GroupSize', headDim, ...
            'OpName', sprintf('h%d.attn.o_proj', layerIdx), 'LayerIndex', layerIdx);
        X_out = qwen2_quant.layer.linear_dispatch(weights.o_proj, attn_output_cat, cfg, projMeta);
        X_out_packed = [];
    elseif useQuantSimOutputPack
...
function layerIdx = getCfgLayerIndex(cfg)
    layerIdx = -1;
    if isfield(cfg, 'LayerIndex')
        layerIdx = double(cfg.LayerIndex);
    end
end
