function [Y, Y_packed, trace] = gatedMLP(X, weights, cfg)
...
%   Uses quantized_matmul for weight operations
    ...
    if strcmpi(string(cfg.LinearMode), "mixed_dynamic")
        gate = qwen2_quant.layer.linear_dispatch(weights.gate_proj, X_flat, cfg, struct('OpName', sprintf('h%d.mlp.gate_proj', layerIdx), 'LayerIndex', layerIdx));
        up = qwen2_quant.layer.linear_dispatch(weights.up_proj, X_flat, cfg, struct('OpName', sprintf('h%d.mlp.up_proj', layerIdx), 'LayerIndex', layerIdx));
        gate_packed = [];
        up_packed = [];
    else
    ...
    trace.swiglu = activated;
    
    % Down projection
    if strcmpi(string(cfg.LinearMode), "mixed_dynamic")
        Y = qwen2_quant.layer.linear_dispatch(weights.down_proj, activated, cfg, struct('OpName', sprintf('h%d.mlp.down_proj', layerIdx), 'LayerIndex', layerIdx));
        Y_packed = [];
    elseif usePackedFullChain && isInt4PackedWeight(weights.down_proj)
