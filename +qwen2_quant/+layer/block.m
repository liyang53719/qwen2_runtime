function [h, present, h_packed, blockTrace] = block(h, past, weights, hyperParameters, freqs_cis, cfg)
...
%   for weight matrix operations.
    ...
    [h_attn, present, h_attn_packed, attnTrace] = quantized_attentionGQA(attnInput, past, attnWeights, freqs_cis, hyperParameters, cfg);
    h_attn = dequantizePackedOrFloat(h_attn_packed, h_attn);
    blockTrace.attention = attnTrace;
    ...
    if usePackedFullChain && ~isempty(h_post_norm_packed)
        [h_ffn, h_ffn_packed, mlpTrace] = gatedMLP(h_post_norm_packed, ffnWeights, cfg);
    else
        [h_ffn, h_ffn_packed, mlpTrace] = gatedMLP(h, ffnWeights, cfg);
    end
    h_ffn = dequantizePackedOrFloat(h_ffn_packed, h_ffn);
    blockTrace.mlp = mlpTrace;
