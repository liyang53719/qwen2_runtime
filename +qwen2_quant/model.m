...
%       layerStates - Updated KV cache
    ...
    for i = 1:numLayers
        ...
        state = layerStates{i};
        layerCfg = runtimeCfg;
        layerCfg.LayerIndex = i - 1;
        
        [Z, newState, Z_packed, blockDetail] = qwen2_quant.layer.block(Z_work, state, layerWeights, hp, currentFreqs, layerCfg);
        ...
        if traceTensors
            ...
            tensorTrace.blockDetails{i, 1} = blockDetail;
        end
        if runtimeCfg.TracePrecision
    ...
    if strcmpi(string(runtimeCfg.LinearMode), "mixed_dynamic")
        logits = qwen2_quant.layer.linear_dispatch(weights.lm_head, Z_flat_float, runtimeCfg);
    elseif isa(weights.lm_head, 'qwen2_quant.internal.quantized_weight')
    ...
    debugInfo.ExecutionTrace = qwen2_quant.internal.execution_trace('get');
    if traceTensors
